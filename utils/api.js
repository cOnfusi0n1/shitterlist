...
import { API_ONLY, runAsync, slWarn, slInfo, slSuccess, showApiSyncMessage, withPrefix, ALLOWED_FLOORS_HELP } from './core';
...
export function apiAddShitterDirect(username, reason, floor){ if(!settings.enableAPI || !settings.apiUrl){ slWarn('API nicht konfiguriert'); return null;} if(!username) return null; const lower=username.toLowerCase(); if(apiPlayersCache.find(p=>p.name.toLowerCase()===lower)){ slInfo(`${username} bereits im API Cache`); return null; } const safeReason = (reason && String(reason).trim().length>0) ? String(reason).trim() : 'Added'; const temp={ id:'pending_'+Math.random().toString(36).substring(2,8), name:username, reason:safeReason, floor: floor, category:'general', severity:1, dateAdded:Date.now(), source:'api', pending:true }; apiPlayersCache.push(temp); slInfo(`API Add gesendet: ${username}`); if(settings.enableWebhook && settings.webhookSendAdds) sendWebhook(`➕ Hinzugefügt: ${code(username)} (${code(safeReason)})`); runAsync('apiAdd',()=>{ const pl={ name:username, reason:safeReason, category:'general', severity:1 }; if(floor) pl.floor=floor; makeAPIRequest('/api/v1/players/batch','POST',{ players:[pl]},(err,res)=>{ if(err||!res||!res.success){ temp.failed=true; const raw=(err&&err.message)?String(err.message):''; if(/Floor must be F1-F7 or M1-M7/i.test(raw)){ ChatLib.chat(withPrefix(ALLOWED_FLOORS_HELP,'warning')); } else { slWarn('Add fehlgeschlagen.'); } return;} slSuccess(`API Add bestätigt: ${username}`); downloadFromAPI(()=>{}); }); }); return temp; }
